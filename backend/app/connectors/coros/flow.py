"""COROS OAuth flow — same state/single-use model as Whoop/Oura."""
...
_STATE_KEY = "coros:oauth:state:{state}"


class OAuthFlowError(Exception):
    """User-facing flow failure (bad state, provider error, no config)."""


def flow_settings_ready() -> bool:
    settings = get_settings()
    return bool(settings.coros_client_id and settings.coros_client_secret)
    ...
    integration = (
        await session.scalars(
            select(Integration).where(
                Integration.user_id == user_id, Integration.provider == "coros"
            )
        )
    ).first()
    if integration is None:
        integration = Integration(user_id=user_id, provider="coros")
        session.add(integration)
    integration.status = "active"
    integration.consecutive_failures = 0
    integration.credentials_encrypted = encrypt_json(tokens.as_credentials())
    await session.commit()
    return {"integration_id": integration.id, "provider": "coros"}
